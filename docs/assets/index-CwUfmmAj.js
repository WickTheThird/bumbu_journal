...
`,clear:()=>{const a=f.querySelectorAll(".output-line"),s=h.length;for(let e=a.length-1;e>=s;e--)a[e].remove();return null},github:()=>(window.open(n.github,"_blank"),'<span class="green">Opening GitHub profile...</span>'),linkedin:()=>(window.open(n.linkedin,"_blank"),'<span class="green">Opening LinkedIn profile...</span>'),echo:a=>a.join(" "),date:()=>new Date().toLocaleString(),pwd:()=>"/home/wick",neofetch:()=>`
...
`},h=["",'<span class="ascii-logo">'+w+"</span>","",'<span class="dim">────────────────────────────────────────────────────────────</span>',"",`  <span class="green">WICK TERMINAL</span> <span class="dim">v3.0.0</span>  <span class="yellow">│</span>  <span class="white">${n.name}</span>  <span class="yellow">│</span>  <span class="dim">${n.title}</span>`,"",`  <span class="green">${n.status}</span>`,"",'<span class="dim">────────────────────────────────────────────────────────────</span>',"",'  <span class="yellow">⚡</span> Type <span class="green">help</span> to see available commands','  <span class="yellow">⚡</span> Type <span class="green">about</span> to learn more about me','  <span class="yellow">⚡</span> Type <span class="green">proj</span> to see my work',""];document.querySelector("#app").innerHTML=`
...
`;const f=document.getElementById("output"),c=document.getElementById("commandInput"),y=document.getElementById("terminal");function o(a){const s=document.createElement("div");s.className="output-line",s.innerHTML=a,f.appendChild(s),y.scrollTop=y.scrollHeight}let g=0;function b(){g<h.length&&(o(h[g]),g++,setTimeout(b,25))}b();const u=[];let p=-1;c.addEventListener("keydown",async a=>{if(a.key==="Enter"){const s=c.value.trim();if(s){u.unshift(s),p=-1,o(`<span class="prompt">wick@portfolio:~$</span> ${s}`);const[e,...i]=s.toLowerCase().split(" ");if(d[e]){const t=await d[e](i);t&&o(t)}else o(`<span class="red">Command not found: ${e}</span>
<span class="dim">Type 'help' for available commands</span>`)}c.value=""}else if(a.key==="ArrowUp")a.preventDefault(),p<u.length-1&&(p++,c.value=u[p]);else if(a.key==="ArrowDown")a.preventDefault(),p>0?(p--,c.value=u[p]):(p=-1,c.value="");else if(a.key==="Tab"){a.preventDefault();const s=c.value.toLowerCase(),e=Object.keys(d).find(i=>i.startsWith(s));e&&(c.value=e)}});y.addEventListener("click",()=>c.focus());document.addEventListener("keydown",a=>{!a.ctrlKey&&!a.metaKey&&!a.altKey&&c.focus()});
